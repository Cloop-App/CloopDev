	...
	router.post('/:topicId/message', authenticateToken, async (req, res) => {
		...
		try {
			...
			for (let i = 0; i < (aiResponse.messages || []).length; i++) {
				...
				// 🔧 FIX: Handle session_summary as TRANSIENT (Do not save to DB)
				if (aiMsg.message_type === 'session_summary') {
					const metricsData = aiMsg.session_metrics || aiResponse.session_metrics;
					console.log('✅ Intercepted session_summary in normal flow - keeping it transient', { metricsFound: !!metricsData });

					const transientMsg = {
						id: Date.now() + Math.floor(Math.random() * 1000),
						sender: 'ai',
						message: aiMsg.formatted_summary || aiMsg.message || 'session_complete',
						message_type: 'session_summary',
						options: aiMsg.options || ['End Session', 'Learn More'],
						...
						session_metrics: metricsData
					};
					aiMessages.push(transientMsg);
